...
$UserAssignedIdentity = Get-AzUserAssignedIdentity -ResourceGroupName $VMResourceGroup -Name $participantName
$participantId = $UserAssignedIdentity.Id
$participantClientId = $UserAssignedIdentity.ClientId
$identityNamePrincipalId = $UserAssignedIdentity.PrincipalId
...
Write-Output "Connect to Azure using:   Connect-AzAccount -Identity -AccountId $participantClientId"
...
Write-Output "Installing Migrate module"
$InvokeCommandResult = Invoke-AzVMRunCommand -ResourceGroupName $VMResourceGroup -VMName $ParticipantVM.Name -CommandId 'RunPowerShellScript' -ScriptPath './tools/InstallModule.ps1' -Parameter @{ModuleName = "Az.Migrate"; ModuleVersion = "0.1.0"}
If ($InvokeCommandResult.Status -eq "Succeeded")
{
  Write-Output "Installation succeded. ${InvokeCommandResult.Value[0].Message}"
}
...
$InvokeCommandResult = Invoke-AzVMRunCommand -ResourceGroupName $VMResourceGroup -VMName $ParticipantVM.Name -CommandId 'RunPowerShellScript' -ScriptPath './tools/ChromeSettings.ps1' -Parameter @{ChromeHomePage = "https://docs.microsoft.com/en-us/azure/"}
If ($InvokeCommandResult.Status -eq "Succeeded")
{
  Write-Output "Installation succeded. ${InvokeCommandResult.Value[0].Message}"
}
